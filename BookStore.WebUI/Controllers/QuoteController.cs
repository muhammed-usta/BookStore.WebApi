...
namespace BookStore.WebUI.Controllers
{
    public class QuoteController : Controller
    {
        ...
        public async Task<IActionResult> QuoteList()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7293/api/Quotes");
        ...
        public async Task<IActionResult> CreateQuote(CreateQuoteDto createQuoteDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7293/api/Quotes", content);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7293/api/Quotes/GetQuote?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateQuote(UpdateQuoteDto updateQuoteDto)
        {
            ...
            StringContent content = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7293/api/Quotes", content);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("QuoteList");
            }
        ...
        public async Task<IActionResult> DeleteQuote(int id)
        {
            ...
            var client = _httpClientFactory.CreateClient();
            await client.DeleteAsync("https://localhost:7293/api/Quotes?id=" + id);
            return RedirectToAction("QuoteList");
        }
